...
public class PermissionsRationaleDialog : MonoBehaviour
{
    ...
    const int kDialogHeight = 600;
    private bool windowOpen = true;

    void DoMyWindow(int windowID)
    {
        GUI.Label(new Rect(10, 20, kDialogWidth - 20, kDialogHeight - 50), "Please let me use the microphone.");
        GUI.Button(new Rect(10, kDialogHeight - 30, kDialogWidth / 4, kDialogHeight/4), "No");
        if (GUI.Button(new Rect(kDialogWidth - 110, kDialogHeight - 30, kDialogWidth / 4, kDialogHeight / 4), "Yes"))
        {
#if PLATFORM_ANDROID
